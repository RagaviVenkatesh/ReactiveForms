...
@Component({
  ...
  styleUrls: ['./multi-checkbox-example.component.css']
})
export class MultiCheckboxExampleComponent implements OnInit {
  ...
  getSelectedHobbies() {
    this.selectedHobbiesNames = this.personForm.controls.hobbies["controls"].map(
      (hobby, i) => {
  ...
  getSelectedHobbiesName() {
    this.selectedHobbiesNames = _.filter(this.selectedHobbiesNames,function(hobby) {
        if (hobby !== false) {
          return hobby;
        }
      }
